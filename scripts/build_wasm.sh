...
echo "   wasm-opt pkg/claude_agent_sdk_rs_bg.wasm -O3 -o pkg/claude_agent_sdk_rs_bg_opt.wasm"
echo ""

# Optional: Optimize with wasm-opt if available
if command -v wasm-opt &> /dev/null; then
    echo -e "${YELLOW}Opt WASM optimization available${NC}"
    echo "Run: wasm-opt pkg/claude_agent_sdk_rs_bg.wasm -O3 -o pkg/optimized.wasm"
fi
