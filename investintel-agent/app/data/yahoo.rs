...
impl YahooFinanceClient {
    ...
    fn parse_quote(&self, symbol: &str, response: YahooChartResponse) -> Result<QuoteData> {
        ...
        let meta = result.meta.ok_or_else(|| anyhow::anyhow!("No meta in chart result"))?;
        ...
        Ok(QuoteData {
           ...
           52_week_high: meta.fifty_two_week_high.unwrap_or(0.0),
            52_week_low: meta.fifty_two_week_low.unwrap_or(0.0),
    ...
    fn parse_historical(&self, symbol: &str, response: YahooChartResponse) -> Result<Vec<OHLCV>> {
        ...
        let timestamp = result.timestamp.ok_or_else(|| anyhow::anyhow!("No timestamp data"))?;
        let quote = result
            .indicators
            .quote
            .first()
...
struct YahooMeta {
    ...
    pub 52_week_high: Option<f64>,
    pub 52_week_low: Option<f64>,
